...
constexpr uintptr_t INTRO_STAGE_ADDR = 0x7CCA20;
constexpr uintptr_t FAKK_CONSOLE_PTR_ADDR = 0x7CCA54;
constexpr uintptr_t UI_PAUSED = 0x7CCA64;
...
constexpr uintptr_t IN_GUIMOUSE = 0x14EB498;
...
static int __cdecl Cvar_Set_Hook(const char* var_name, const char* value, int flag)
{
	...
	if (MemoryHelper::ReadMemory<int>(INTRO_STAGE_ADDR) == -1)
	{
...
static int __cdecl PushMenu_Hook(const char* menu_name)
{
	...
	if (!isTitleBgSet && UseConsoleTitleScreen && strcmp(menu_name, "main") == 0)
	{
		...
		int start_state = MemoryHelper::ReadMemory<int>(INTRO_STAGE_ADDR);
...
static int __stdcall lpfnWndProc_MSG_Hook(HWND hWnd, UINT Msg, int wParam, LPARAM lParam)
{
	...
	{
		if (!MemoryHelper::ReadMemory<int>(IN_GUIMOUSE))
		{
...
static int __cdecl Draw_StretchPic_Hook(float x_position, float y_position, float resolution_width, float resolution_height, float a5, float a6, float a7, float a8, int ShaderHandle)
{
	...
	{
		BYTE** ptr = (BYTE**)FAKK_CONSOLE_PTR_ADDR;
...
static void ApplyFixSoundRandomization()
{
	...
	{
		// sub_423770 (Alias_ListFindRandomRange)
		...
		// sub_423200 (Alias_ListUpdateDialog)
		...
		// sub_423270 (Alias_ListAddActorDialog)
		...
		// sub_4236C0 (Alias_ListFindRandom)
...
static void ApplyFixMenuAnimationSpeed()
{
	...
	// Read 640x480 instead of the current resolution (UIWidget::Motion)
...
static void ApplyUseOriginalIntroVideos()
{
	...
	// Instructions from the 2000 version, ported to the remaster (UI_DoIntro_f)
